...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($newPassword !== $confirmPassword) {
        ...
        $stmt = $conn->prepare("UPDATE registerusers SET password = ? WHERE email = ?");
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('✅ Password updated successfully.'); window.location='../auseregisterlogform/login.php';</script>";
            exit;
        } else {
...
<body class="bg-light d-flex align-items-center justify-content-center min-vh-100">
    <div class="card shadow-lg border-0 rounded-4 p-4" style="max-width: 400px; width: 100%;">
        <a href="forgot-password.php" class="back-circle" aria-label="Go back">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn btn-yellow w-100">Reset Password</button>
        </form>
        <script>
            ...
            document.getElementById('togglePassword1').addEventListener('click', function(){ togglePassword('password','pwIcon1'); });
            document.getElementById('togglePassword2').addEventListener('click', function(){ togglePassword('confirm_password','pwIcon2'); });
        </script>
    </div>
</body>
</html>
